...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    if (req.method === 'GET') {
      // Query from cai_hackathons_gallery table - fetch hackathon, laboratories, and placements categories
      const rows: any = await executeQuery(
        "SELECT id, dept, category, academic_year, gallery, title FROM cai_hackathons_gallery WHERE category IN ('hackathon', 'laboratories', 'placements') ORDER BY category ASC, academic_year DESC"
      );
      ...
      await executeQuery(
        "UPDATE cai_hackathons_gallery SET dept = ?, category = ?, academic_year = ?, title = ?, gallery = ? WHERE id = ?",
