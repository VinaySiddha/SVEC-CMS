...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    if (req.method === 'GET') {
      // Query from cst_hackathons_gallery table - fetch hackathon, laboratories, labs, and placements categories
      const rows: any = await executeQuery(
        "SELECT id, dept, category, academic_year, gallery, title FROM cst_hackathons_gallery WHERE category IN ('hackathon', 'laboratories', 'labs', 'placements') ORDER BY category ASC, academic_year DESC"
      );
      ...
      const result: any = await executeQuery(
        "INSERT INTO cst_hackathons_gallery (dept, category, academic_year, title, gallery) VALUES (?, ?, ?, ?, ?)",
        [dept || 'cst-ai', category, academic_year, title || null, gallery]
      );
      ...
      await executeQuery(
        "UPDATE cst_hackathons_gallery SET dept = ?, category = ?, academic_year = ?, title = ?, gallery = ? WHERE id = ?",
        [dept || 'cst-ai', category, academic_year, title || null, gallery, id]
      );
      ...
      await executeQuery("DELETE FROM cst_hackathons_gallery WHERE id = ?", [id]);
