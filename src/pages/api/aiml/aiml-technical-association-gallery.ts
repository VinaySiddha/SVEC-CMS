...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    if (req.method === 'GET') {
      // Fetch from aiml_hackathons_gallery with category = 'technical association'
      const rows: any = await executeQuery(
        "SELECT id, dept, category, academic_year, gallery, title FROM aiml_hackathons_gallery WHERE category = 'technical association' ORDER BY academic_year DESC"
      );
      ...
      const result: any = await executeQuery(
        "INSERT INTO aiml_hackathons_gallery (dept, category, academic_year, title, gallery) VALUES (?, 'technical association', ?, ?, ?)",
        [dept || 'aiml', academic_year, title || null, gallery]
      );
      ...
      await executeQuery(
        "UPDATE aiml_hackathons_gallery SET dept = ?, academic_year = ?, title = ?, gallery = ? WHERE category='technical association' AND id = ?",
        [dept || 'aiml', academic_year, title || null, gallery, id]
      );
      ...
      await executeQuery("DELETE FROM aiml_hackathons_gallery WHERE id = ?", [id]);
