...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    if (req.method === 'GET') {
      // Query from aiml_hackathons_gallery table - fetch both hackathon and laboratory categories
      ...
      const result: any = await executeQuery(
        "INSERT INTO aiml_hackathons_gallery (dept, category, academic_year, title, gallery) VALUES (?, 'hackathon', ?, ?, ?)",
      ...
      await executeQuery(
        "UPDATE aiml_hackathons_gallery SET dept = ?, academic_year = ?, title = ?, gallery = ? WHERE id = ?",
