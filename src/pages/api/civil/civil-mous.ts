  ...
  try {
    if (req.method === 'GET') {
      const rows: any = await executeQuery(
        "SELECT * FROM civil_mous ORDER BY from_date DESC"
      );
      ...
      const normalized = (rows || []).map((r: any) => ({
        ...
        // status may not exist on older cst_mous table — default to null/empty
    ...
    console.error('Civil MOUs API Error:', error);
    res.status(500).json({
