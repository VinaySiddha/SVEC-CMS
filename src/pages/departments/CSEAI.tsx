...
const getDesignationPriority = (designation: string): number => {
  ...
  if (designationLower.includes('assoc. professor')) return 3;
  if (designationLower.includes('assoc professor')) return 3;

  // 4. Senior Assistant Professor
  if (designationLower.includes('sr.') && designationLower.includes('asst.') && designationLower.includes('professor')) return 4;
  if (designationLower.includes('sr') && designationLower.includes('asst') && designationLower.includes('professor')) return 4;
  if (designationLower.includes('senior assistant professor')) return 4;

  // 5. Assistant Professor
  if (designationLower.includes('asst.') && designationLower.includes('professor')) return 5;
  if (designationLower.includes('asst') && designationLower.includes('professor')) return 5;
...
const CSTDepartment: React.FC = () => {
  ...
  const [technicalAssociationData, setTechnicalAssociationData] = useState<any[]>([]);
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      setExtracurricularActivities(Array.isArray(extracurricularActivitiesData) ? extracurricularActivitiesData : []);
      setTechnicalAssociationData(Array.isArray(technicalAssociationData) ? technicalAssociationData : []);
          ...
          <div className="bg-white p-6 md:p-8 rounded-2xl shadow-lg animate-fade-in">
            ...
            <div className="space-y-6">
              {grouped.map((group, index) => (
                <details key={`${group.category}-${index}`} open={index === 0} className="cst-dropdown">
                      ...
                      <tbody>
                        {bosMembers.map((member, idx) => (
                          <tr key={member.id} className="hover:bg-gray-50">
                            ...
                            <td className="py-3 px-4 border-b border-gray-200">{(member as any).organization || ''}</td>
                            <td className="py-3 px-4 border-b border-gray-200">{(member as any).position_in_job}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        value={(editingMou as any)?.organization_name || ''}
                        onChange={(e) => setEditingMou({ ...(editingMou as any), organization_name: e.target.value })}
        ...
        return (
          <div className="bg-white p-6 md:p-8 rounded-2xl shadow-lg animate-fade-in">
            ...
            <div className="space-y-4">
              ...
              {(() => {
                const eapcetRankers = technicalAssociationData && technicalAssociationData.length > 0
                  ? technicalAssociationData.filter((item: any) => item.category && item.category.toLowerCase() === 'eapcet')
                  : [];
              ...
              {(() => {
                const engineersDay = technicalAssociationData && technicalAssociationData.length > 0
                  ? technicalAssociationData.filter((item: any) => item.category && item.category.toLowerCase() === 'engday')
                  : [];
      ...
      case 'Extra-Curricular Activities': {
        ...
        return (
          <div className="bg-white p-6 md:p-8 rounded-2xl shadow-lg animate-fade-in">
            ...
            <div className="space-y-6">
              {(() => {
                // Filter extracurricularActivities with category 'extra curricular'
                const extraCurricularItems = extracurricularActivities && extracurricularActivities.length > 0
                  ? extracurricularActivities.filter((item: any) => item.category && item.category.toLowerCase() === 'extra curricular')
                  : [];
                ...
                <div className="cst-dropdown-content">
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      {(() => {
                        // Filter extracurricularActivities with category 'Maitri Coordinators'
                        const maitriCoordinators = extracurricularActivities && extracurricularActivities.length > 0
