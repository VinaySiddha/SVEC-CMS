...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = 'SELECT * FROM exam_section_noticeboard';
    let params: any[] = [];
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { id, title, category, content, file_url, posted_date } = body;
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const query = 'DELETE FROM exam_section_noticeboard WHERE id = ?';
