...
export default function AdminEditFDPPage() {
  const params = useParams();
  const router = useRouter();
  const fdpId = Array.isArray(params?.id) ? params.id[0] : (params?.id || '');
  ...
  useEffect(() => {
    const fetchDepartmentsAndFDP = async () => {
      try {
        // Fetch departments
        ...
        // Fetch faculty members for the FDP's department
        try {
          if (fdpData.dept) {
            const facultyResponse = await fetch(`/api/faculty?dept=${fdpData.dept}`);
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">
          Edit Faculty Development Program
        </h1>
