...
async function migrateECTClubs() {
  const connection = await mysql.createConnection(dbConfig);

  try {
    // Create table if not exists
    await connection.execute(`
      CREATE TABLE IF NOT EXISTS ect_clubs (
    ...
    `);
    console.log('✅ ect_clubs table created/verified successfully');

    // Clear existing data (optional)
    await connection.execute('DELETE FROM ect_clubs');
    console.log('🗑️ Cleared existing ECT clubs records');
    ...
    for (const item of clubsData) {
      try {
        await connection.execute(
          `INSERT INTO ect_clubs (club, event, description, url, created_at) VALUES (?, ?, ?, ?, NOW())`,
    ...
    // Verify the data
    const [rows] = await connection.execute('SELECT COUNT(*) as count FROM ect_clubs');
...
if (require.main === module) {
  migrateECTClubs()
    .then(() => {
      console.log('🎉 ECT Clubs migration completed!');
      process.exit(0);
    })
    .catch((error) => {
...
module.exports = { migrateECTClubs, clubsData };
