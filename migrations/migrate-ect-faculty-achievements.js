...
async function migrateECTFacultyAchievements() {
  const connection = await mysql.createConnection(dbConfig);

  try {
    // Create table if not exists
    await connection.execute(`
      CREATE TABLE IF NOT EXISTS ect_faculty_achievements (
    ...
    `);
    console.log('✅ ect_faculty_achievements table created/verified successfully');

    // Clear existing data (optional)
    await connection.execute('DELETE FROM ect_faculty_achievements');
    ...
    for (const item of achievementsData) {
      try {
        await connection.execute(
          `INSERT INTO ect_faculty_achievements (type, year, title, url, details, created_at) VALUES (?, ?, ?, ?, ?, NOW())`,
    ...
    // Verify the data
    const [rows] = await connection.execute('SELECT COUNT(*) as count FROM ect_faculty_achievements');
...
if (require.main === module) {
  migrateECTFacultyAchievements()
    .then(() => {
      console.log('🎉 ECT Faculty Achievements migration completed!');
      process.exit(0);
    })
    .catch((error) => {
...
module.exports = { migrateECTFacultyAchievements, achievementsData };
